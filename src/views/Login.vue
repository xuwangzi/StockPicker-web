<template>
  <div class="login-wrapper" id="loginBackground">
    <div class="form-wrapper">
      ...
      <p class="login-system">StockPicker</p>
      <!-- 登录表单 -->
      <el-form
        class="login-form"
        ref="loginForm"
      ...
      >
        ...
        <el-form-item class="login-btn-form-item">
          <el-button class="login-btn" type="primary" :disabled="loginBtnDisabled" @click="submitForm('loginForm')">登录</el-button>
        </el-form-item>
        <el-form-item class="login-btn-form-item">
          <el-button class="login-btn" type="primary" @click="goToRegister()">还没账号？去注册</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
// 配置：new CanvasNest(element, config)
const config = {
...
export default {
  ...
  computed: {
    url() {
      ...
      console.log(_url)// http://localhost:8081/login?Rurl=%2Fstock
      return _url ? { path: _url } : { name: 'Stock' }  //  若登录之前有页面，则登录后仍然进入该页面
    }
  },
  watch: {
    //  滑动解锁验证通过时，若重新输入用户名或密码，滑动解锁和登录按钮恢复原样
    'loginForm.telephone'() {
  ...
  methods: {
    /**
     * 跳转到注册页面
     * @description 在登录页面跳转到注册页面
     */
    goToRegister() {
      this.$message.success('请注册^_^')
    ...
    updateIsPassing(isPassing) {
      if (isPassing) {
        //  校验手机号
        this.$refs.loginForm.validateField('telephone', (telephoneError) => {
          if (telephoneError) {
            this.loginBtnDisabled = true
          } else {
            //  校验密码
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        // 箭头函数：作为 validate 方法的回调函数。当验证完成时，这个函数会被调用。
        if (valid) {
          // 表单各项校验通过
